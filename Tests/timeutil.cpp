...
make_pair("duration stream writing", [] {
    KSS_ASSERT(isEqualTo<string>("10h", [] {
        ostringstream oss;
        oss << 10h;
        return oss.str();
    }));
    KSS_ASSERT(isEqualTo<string>("10min", [] {
        ostringstream oss;
        oss << 10min;
        return oss.str();
    }));
    KSS_ASSERT(isEqualTo<string>("10s", [] {
        ostringstream oss;
        oss << 10s;
        return oss.str();
    }));
    KSS_ASSERT(isEqualTo<string>("10ms", [] {
        ostringstream oss;
        oss << 10ms;
        return oss.str();
    }));
    KSS_ASSERT(isEqualTo<string>("10us", [] {
        ostringstream oss;
        oss << 10us;
        return oss.str();
    }));
    KSS_ASSERT(isEqualTo<string>("10ns", [] {
        ostringstream oss;
        oss << 10ns;
        return oss.str();
    }));
    KSS_ASSERT(isEqualTo<string>("0.5s(approx)", [] {
        ostringstream oss;
        chrono::duration<int, ratio<1, 2>> d(1);
        oss << d;
        return oss.str();
    }));
}),
make_pair("duration stream reading", [] {
    KSS_ASSERT(isEqualTo<chrono::hours>(10h, [] {
        istringstream iss("10h");
        chrono::hours h;
        iss >> h;
        return h;
    }));
    KSS_ASSERT(isEqualTo<chrono::minutes>(10min, [] {
        istringstream iss("10min");
        chrono::minutes m;
        iss >> m;
        return m;
    }));
    KSS_ASSERT(isEqualTo<chrono::seconds>(10s, [] {
        istringstream iss("10s");
        chrono::seconds s;
        iss >> s;
        return s;
    }));
    KSS_ASSERT(isEqualTo<chrono::milliseconds>(10ms, [] {
        istringstream iss("10ms");
        chrono::milliseconds ms;
        iss >> ms;
        return ms;
    }));
    KSS_ASSERT(isEqualTo<chrono::microseconds>(10us, [] {
        istringstream iss("10us");
        chrono::microseconds us;
        iss >> us;
        return us;
    }));
    KSS_ASSERT(isEqualTo<chrono::nanoseconds>(10ns, [] {
        istringstream iss("10ns");
        chrono::nanoseconds ns;
        iss >> ns;
        return ns;
    }));
    KSS_ASSERT(throwsException<istringstream::failure>([] {
        istringstream iss("bad value");
        iss.exceptions(istringstream::failbit);
        chrono::seconds s;
        iss >> s;
    }));
    KSS_ASSERT(throwsException<istringstream::failure>([] {
        istringstream iss("10unsupported duration");
        iss.exceptions(istringstream::failbit);
        chrono::duration<int, ratio<1, 2>> d;
        iss >> d;
    }));
    KSS_ASSERT(throwsException<istringstream::failure>([] {
        istringstream iss("10wrong suffix");
        iss.exceptions(istringstream::failbit);
        chrono::seconds s;
        iss >> s;
    }));
